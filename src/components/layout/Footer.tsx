...
const Footer = () => {
  ...
  return (
    <footer className="bg-white text-[#0E3B6E] pt-12 pb-8 border-t-4 border-yellow-400">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
          {/* About Section */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-yellow-600">MBL Ceará</h3>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {menuItems.map((item) => (
                <li key={item.path}>
                  <Link href={item.path} className="text-gray-300 hover:text-primary transition-colors">
                    {item.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-yellow-200 pt-6">
          <p className="text-center text-yellow-700 text-sm">
            © {currentYear} MBL Ceará. Todos os direitos reservados.
          </p>
        </div>
      </div>
    </footer>
